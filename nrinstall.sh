...
tgta18=18.19.0   # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
tgtl18=18.4.0    # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
tgta20=20.11.0    # need armv6l latest from https://unofficial-builds.nodejs.org/download/release/
# tgtl20=None      # need x86 latest from https://unofficial-builds.nodejs.org/download/release/
...
case $yn in
    [Yy]* )
        ...
        if [[ "$SYSTEMDOK" == "true" ]]; then
            ...
            $SUDO sed -i 's#^User=.*#User='$NODERED_USER'#;s#^Group=.*#Group='$NODERED_GROUP'#;s#^WorkingDirectory=.*#WorkingDirectory='$NODERED_HOME'#;s#^EnvironmentFile=-.*#EnvironmentFile=-'$NODERED_HOME'/.node-red/EnvironmentFile#;' ${SYSTEMDFILE}.temp
